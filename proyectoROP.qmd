...
## Clasficación binaria

```{r Clasificacion binaria}
# Cargar las bibliotecas necesarias
library(pROC)

# Dividir los datos en conjuntos de entrenamiento y prueba
set.seed(123)
# trainIndex <- sample(1:nrow(rentabilidadAnu), 0.7 * nrow(rentabilidadAnu))
data_train <- rentabilidadAnu[1:90, ]
data_test <- rentabilidadAnu[91:180, ]

# Crear una variable objetivo binaria (por ejemplo, clasificar si 'anu_bac' es mayor o igual al 0)
data_train$binary_target <- ifelse(data_train$anu_bac >= 0, 1, 0)
data_test$binary_target <- ifelse(data_test$anu_bac >= 0, 1, 0)

# Construir un modelo de clasificación binaria (por ejemplo, regresión logística)
model <- glm(binary_target ~ ., data = data_train, family = binomial)

# Realizar predicciones en los datos de prueba
predictions <- predict(model, newdata = data_test, type = "response")

# Crear una matriz de confusión
conf_matrix <- table(Actual = data_test$binary_target, Predicted = ifelse(predictions > 0.5, 1, 0))

# Generar una curva ROC con etiquetas en español
roc_curve <- roc(data_test$binary_target, predictions)
